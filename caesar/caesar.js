...
function getNewCode (code, shift, charCase) {
	let shiftAmountBeyondLetterA = code + shift - charCase;
	let finalCode = (shiftAmountBeyondLetterA % 26 + 26) % 26 + charCase;
	return finalCode;
}
...
function shiftCharacter (char, shift) {
	...
	let shiftedChar = getNewCode(code, shift, charCase);  // wrap around alphabet if necessary (the % gets the amount of shift, starting from the letter 'a', and charCase starts the count from 'a'). If the shift is past the last letter, it starts the count from 'a'. If it doesn't wrap around, you're getting the same number you would get if you added the charCode and shift together anyway. 
